      ...
      <Paper className={classes.root}>
        <Table className={classes.table}>
          ...
          <TableBody>
            {data
              .filter(e => {
                return e.tracking !== "DISBURSTMENT";
              })
              .map((row, i) => (
                <TableRow className={classes.row} key={i}>
                  ...
                  <CustomTableCell align="left">
                    <Link to={`/admin/loan/${row._loan}`}>
                      {row.borrower[0].firstName +
                        " " +
                        row.borrower[0].lastName}
                    </Link>
                  </CustomTableCell>
                  ...
                  <CustomTableCell align="right">
                    {parseInt(row.balance).toFixed(2)}
                  </CustomTableCell>
                  <CustomTableCell align="right">
                  ...
                  <CustomTableCell align="right">
                    {lateFeeHandle(row.date, row.interest_pmt)
                      ? lateFeeCalc(row.date, row.principal).toFixed(2)
                      : ""}{" "}
                  </CustomTableCell>
                  <CustomTableCell align="right">
                    {parseInt(row.payment).toFixed(2)}
                  </CustomTableCell>
                </TableRow>
              ))}
          </TableBody>
